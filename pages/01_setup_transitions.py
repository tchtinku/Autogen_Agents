...
if st.session_state.able_to_run:
    ...
    with c1:
        if st.button("⏏️ Load transitions"):
            ...
            st.session_state.saved_transitions = json.load(open("agent_transitions.json"))
    ...
    if len(st.session_state.saved_transitions) == 0:
        ...
        _agents_dict = {agents["name"]: agent for agent in session_state.saved_agents}
        ...
        if st.button("Use these and Continue", type="primary"):
