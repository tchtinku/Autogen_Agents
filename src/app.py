...
from magnetic_one_helper import MagneticOneHelper
...
st.title("MagneticOne Workflow Runner")
...
def display_log_message(log_entry):
    ...
    if _type == "OrchestrationEvent":
        ...
        st.caption(f'{_timestamp} LLM Call [prompt_tokens: {_log_entry_json["prompt_tokens"]}, complete_tokens: {_log_entry_json["completion_tokens"]}]')
    else:
        st.caption("Invalid og entry format")
...
async def main(task, logs_dir="./logs"):
    ...
    # Initialize MagneticOne
    magnetic_one = MagneticOneHelper(logs_dir=logs_dir)
    await magnetic_one.initialize()
    print("MagneticOne initialized.")
